...
public class UpdateService {
    ...
    @Transactional
    public int update() {
        List<Link> links = jooqLinkRepository.findByOldestUpdates(COUNT_UPDATES);
        int count = 0;
        for (Link link : links) {
            ...
            jooqLinkRepository.setUpdateAt(link.getUrl().toString(), OffsetDateTime.now(ZoneOffset.UTC));
        }
        return count;
    }
    ...
    public void deleteUnusedLinks() {
        jooqLinkRepository.deleteUnusedLinks();
    }
}
